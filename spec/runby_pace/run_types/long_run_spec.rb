...
describe Runby::RunTypes do
  ...
  describe runs::LongRun do
    ...
    it 'has a nice explanation' do
      expect(runs::LongRun.new.explanation.length).to be > 50
    end
    ...
    it 'calculates a set of long run (fast) paces correctly' do
      ...
      runs::LongRun::GoldenPaces.fast.each do |five_k, golden_pace|
        calculated_pace_range = long_run.lookup_pace(five_k)
    ...
    it 'calculates the pace in minutes per mile' do
      long_run = runs::LongRun.new
      calculated_pace_range = long_run.lookup_pace('14:00', :mi)
      expect(calculated_pace_range.fast.to_s(format: :long)).to eq '6:26 per mile'
    end
  end
end
